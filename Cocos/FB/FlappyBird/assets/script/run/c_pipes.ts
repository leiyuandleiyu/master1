...
export default class c_pipes extends cc.Component {
    ...
    private dis_x_max:number = 200;//管道x间隔最大
    private dis_x_min:number = 120;//管道x间隔最小
    private dis_x:number = this.dis_x_max;//管道x间隔
    private dis_x_cu:number = this.dis_x_max;//当前管道x间隔的累积值
    private dis_y_max:number = 120;//管道x间隔最大
    private dis_y_min:number = 80;//管道x间隔最小
    private dis_y:number = this.dis_y_max;//管道y间隔
    ...
    //难度的变化
    moreDiffcult(){
        let cu = c_event.ins.getScoreCu();
        // this.dis_y = cc.clamp(this.dis_y_max - cu*1,this.dis_y_min,this.dis_y_max);
        //调整管道横向间隔
        if((this.dis_x_max - cu*2)>200){
            this.dis_x = 200;
        }else if((this.dis_x_max - cu*2)<120){
            this.dis_x = 120;
        }else{
            this.dis_x = this.dis_x_max - cu*2;
        }
        //调整管道纵向间隔
        if((this.dis_y_max - cu*1) > 120){
            this.dis_y = 120;
        }else if((this.dis_y_max - cu*1) < 80){
            this.dis_y = 80;
        }else{
            this.dis_y = this.dis_y_max - cu*1
        }
